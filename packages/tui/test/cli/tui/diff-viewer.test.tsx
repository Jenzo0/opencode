...
test("closing the diff viewer returns to the route it opened from", async () => {
  ...
  try {
    ...
    await viewer.commands.get("diff.close")!.run?.({} as never)
...
+const newThird = true
 const afterThird = true`,
  ...
  try {
    ...
    await viewer.commands.get("diff.next_hunk")!.run?.({} as never)
    ...
    await viewer.commands.get("diff.previous_hunk")!.run?.({} as never)
    await viewer.app.renderOnce()
